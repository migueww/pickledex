...
export async function fetchExternalEpisode(id: number): Promise<Episode> {
  ...
  try {
    ...
    throw new UpstreamError(500, 'Não foi possível consultar a API. Tente novamente em alguns instantes.')
  }
}
...
export async function fetchExternalCharacters(ids: number[]): Promise<Character[]> {
  ...
  try {
    ...
    // Sort according to original input ids request list
    const idMap = new Map(result.map(c => [c.id, c]))
    return ids.map(id => idMap.get(id)!).filter(Boolean)
  } catch (error) {
    if (error instanceof UpstreamError) throw error
    throw new UpstreamError(500, 'Erro ao buscar personagens. A consulta falhou.')
  }
}
